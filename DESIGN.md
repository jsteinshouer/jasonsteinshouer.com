...
colors:
  ...
  select: "#cfe6dd"
  paper-dark: "#101211"
  paper-sunk-dark: "#191c1a"
  paper-raised-dark: "#171a18"
  ink-dark: "#e4e5e1"
  ink-soft-dark: "#9aa09b"
  ink-faint-dark: "#868d88"
  rule-dark: "#282c29"
  rule-strong-dark: "#3a403c"
  accent-dark: "#59d6ae"
  accent-deep-dark: "#8ee7c9"
  accent-wash-dark: "#17251f"
  select-dark: "#1e4438"
typography:
  display:
...
### Primary
- **Spruce Green** (`{colors.accent}` light / `{colors.accent-dark}` dark): The only hue in the system. It carries links, tag hover, list markers, focus rings, the caret, the `#` heading anchor, the active TOC entry, the blockquote rule, the wordmark's surname, and syntax strings. Nothing decorative ever uses it.
...
### Neutral
- **Warm Paper** (`{colors.paper}`): The page ground and the masthead ground. Off-white, never pure white, in light theme.
...
- **Ink** (`{colors.ink}`): All body text, headings, and titles.
...
- **Headline** (600, `clamp(1.875rem → 2.5rem)`, 1.1, -0.022em): Page heads on tags, archive, projects, about, 404.
- **Title** (600, `clamp(1.5rem → 1.875rem)`, 1.2, -0.02em): Post-list entry titles, project names, archive year numbers, and prose `h2`.
...
- **Wordmark** (serif, 600, 1.1875rem, -0.015em): The masthead identity only, surname in accent. It is a type role, not a logo, and appears on every page.
...
The system is a single centered column with three widths, all derived from one measure. `--measure` is 68ch and sets the prose column on posts, pages, projects, and 404. `--column--index` is `measure + 9rem`, used on the home and tag post lists so the hanging date margin sits *outside* the measure rather than eating into it; the running head on those pages is padded 9rem to align with the entry bodies. `--column--wide` is 54rem, for tag index, archive, and project index — surfaces that are lists, not prose. Every one of them sits inside `.shell`, capped at `--page` (78rem) with a fluid `--gutter` of `clamp(1.25rem, 5vw, 3rem)`.
...
The code panel's horizontal scroll cue is a four-layer background gradient, two layers attached `local` and two `scroll`, so a soft edge fade appears only while there is code past that edge; it is not a shadow and must not be replaced by one. The search overlay reads as lifted because it sits on a `paper`-tinted scrim with `backdrop-filter: blur(6px) saturate(1.2)`, on `paper-raised`, inside a `rule-strong` border — again, no shadow. The masthead is sticky and flat, and announces itself only by fading in its bottom hairline once `scrollY > 8`.
...
- **Non-link variant:** Project technology chips render as `<span>` with the same shape and no hover.
...
- **Don't** turn a list of posts, tags, or projects into cards. Rows and hairlines carry every index in the system.
- **Don't** introduce a second accent hue, including for status, category, or emphasis. See **The One Green Rule**.
